/*
 * BLE peripheral protocol for wifi_gps_softap (canonical for GPS SoftAP demo builds).
 * Mobile mirror: mobile/src/protocol/types.ts (DroneCmd).
 */
#ifndef H_BLEPRPH_
#define H_BLEPRPH_
...
typedef enum {
    ...
    /* Navigation intent (autonomy phase signals from mobile). */
    DRONE_CMD_NAV_ROTATE_CW    = 0x34,
    DRONE_CMD_NAV_ROTATE_CCW   = 0x35,
    DRONE_CMD_NAV_FORWARD      = 0x36,
    DRONE_CMD_NAV_STRAFE_LEFT  = 0x37,
    DRONE_CMD_NAV_STRAFE_RIGHT = 0x38,
    DRONE_CMD_NAV_HOLD         = 0x39,
