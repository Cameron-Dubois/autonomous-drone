...
/** Active BLE connection for telemetry notify (single central). */
static uint16_t s_ble_active_conn = BLE_HS_CONN_HANDLE_NONE;
...
bleprph_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            ...
            pkey.action = event->passkey.params.action;
            /* No console CLI in unified firmware — auto-accept for lab demo. */
            ...
            ESP_LOGI(tag, "BLE_SM_IOACT_INPUT — using zero passkey (demo build)");
            pkey.action = event->passkey.params.action;
            pkey.passkey = 0;
            rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
            ESP_LOGI(tag, "ble_sm_inject_io result: %d", rc);
...
{
    return s_ble_active_conn;
}

void
wifi_gps_ble_stack_start(void)
{
    int rc;
    esp_err_t ret;
