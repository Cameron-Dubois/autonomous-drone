...
- **NimBLE** — Same binary advertises **`DroneBLE`**; GATT writes accept flight commands; notifications send **Base64(JSON)** telemetry aligned with `/gps` when a phone is subscribed. **`DEMO_TAKEOFF` (`0x3C`)** runs an onboard DSHOT motor ramp; other opcodes are log-only for demo. Protocol: [`BLE_PROTOCOL_MOBILE.md`](BLE_PROTOCOL_MOBILE.md).
