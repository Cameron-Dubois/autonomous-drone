...
- **NimBLE** — Same binary advertises **`DroneBLE`**; GATT writes accept flight commands; notifications send **Base64(JSON)** telemetry aligned with `/gps` when a phone is subscribed. Protocol: [`BLE_PROTOCOL_MOBILE.md`](BLE_PROTOCOL_MOBILE.md); sources in `main/bleprph.h` and `main/gatt_svr.c`.
