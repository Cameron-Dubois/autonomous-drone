...
Each subsystem is its own ESP-IDF component. `main/` only orchestrates
boot.
...
├── main/
│   ├── CMakeLists.txt          REQUIRES the high-level components
│   └── app_main.c              boot orchestration only
└── components/
    ├── config/                 ★ header-only: ALL pins, rates, gains
    │   ├── CMakeLists.txt
    │   └── include/app_config.h
    ├── imu/                    MPU6050 driver  (REQUIRES driver config)
    ├── attitude/               complementary-filter estimator  (REQUIRES imu config)
    ├── motors/                 LEDC PWM driver  (REQUIRES driver config)
    ├── pid/                    PID with anti-windup + D-on-measurement  (no deps)
    ├── mixer/                  X-quad mixer w/ throttle-preserving saturation (no deps)
    ├── wifi_link/              SoftAP + UDP control + telemetry
    │                           (REQUIRES esp_wifi esp_netif esp_event nvs_flash
    │                                     lwip esp_timer config)
    └── flight_task/            1 kHz hot path
                                (REQUIRES imu attitude motors pid mixer wifi_link
                                          esp_timer config esp_system)
```

Each component folder follows the same shape:

```
components/<name>/
├── CMakeLists.txt              idf_component_register(...)
├── include/<name>.h            public header (exposed to dependents)
...
```
            ┌──────────┐
            │  config  │  ← header-only, everyone depends on this
            └────┬─────┘
       ┌────────┼─────────┬───────────────┬───────────────┐
       ▼        ▼         ▼               ▼               ▼
     imu     motors    wifi_link        pid            mixer
       │
       ▼
   attitude
       │
...
- **Battery monitor**. Wire a 1:2 divider into ADC1_CH0 and add a
  low-voltage failsafe in `components/flight_task/flight_task.c`.
- **Persistent gain storage**. PIDs init from `components/config/include/app_config.h`;
  storing them in NVS so you can tune over the air is a one-evening change.
